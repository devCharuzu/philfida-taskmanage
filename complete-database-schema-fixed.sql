...
-- Manual Personnel-ID mode (anon JWT): frontend still operates without Supabase Auth session.
-- These policies are broad by design to keep legacy SPA flows working with RLS enabled.
...
-- Profile updates (personnel ID ≠ auth.uid(); manual login is anon)
